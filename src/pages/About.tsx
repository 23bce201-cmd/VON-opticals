...
import shopImage from "../photos/shop/shop3.jpeg";
      ...
      <section className="page-pad bg-white swiss-grid-pattern">
        <div className="container-wide">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-8">
            ...
            <Reveal delay={120}>
              <img src={shopImage} alt="VON Optical frame wall and store display" className="aspect-[4/3] w-full border-4 border-von-blue-900 object-contain " />
            </Reveal>
          </div>
        </div>
      </section>
        ...
        <div className="container-wide">
          <div className="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 md:grid-cols-3 lg:px-8">
            {[
              ...
              <Reveal key={item.title} delay={index * 90} className="group border-4 border-von-blue-900 bg-white p-6 transition hover:bg-von-accent hover:text-white">
                <item.icon className="text-current" size={25} strokeWidth={2.5} />
                <p className="mt-5 swiss-number group-hover:text-white">0{index + 1}. System</p>
                <h2 className="mt-3 font-display text-2xl font-black uppercase leading-none text-current">{item.title}</h2>
                <p className="mt-4 text-sm font-medium leading-6 text-von-blue-900/70 group-hover:text-white/85">{item.copy}</p>
              </Reveal>
            ))}
          </div>
        </div>
